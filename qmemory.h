...
#define qmem_append(dst, item, type) \
    do { \
        ...
        if ((dst->unwritten) > (dst->blklen) - 1) { \
            if ((new_tail = malloc(sizeof(struct qmem_node))) == NULL) { \
                handle_error(ERR_MEM_ALLOC_FAIL); \
            } else if ((new_tail->v = malloc(sizeof(type)*(dst->blklen))) == NULL) { \
                handle_error(ERR_MEM_ALLOC_FAIL); \
            } \
            if (dst->blknum == 0) { \
                new_tail->last = NULL; \
...
static inline bool
qmem_empty(const qmem_t item)
{
    return item != NULL && item->blknum == 0;
}
...
qmem_iter_end(const qmem_iter_t item)
{
    ...
    return (item.source)->tail == item.current_read_blk &&
